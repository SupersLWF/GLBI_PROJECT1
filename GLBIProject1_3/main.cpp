...
GLfloat vertices[] = {
	...
	//0.5f,  -0.5f,  0.0f,		// 1，重复
	//0.0f,   0.5f,  0.0f,		// 2，重复
	0.8f,   0.8f,  0.0f			// 3  ，为点标号后，发现除重复点外，只有四个非重复点，显然，要绘制这两个三角形，只传输这4个点的数据就够了，
	//顺时针排点，					  //不过显然，只有这四个点的数据还是不够的，因为接下来要绘图的OpenGL函数，以及配套参数只能绘制三角形，因此
									  //我们还得给这四个点分组，并告诉OpenGL要画的两组三角形的点 是哪两组。 
									  //如此，就要用到EBO（ELEMENT BUFFER OBJECT,也称IBO: INDEX BUFFER OBJECT）了。
									
	//顺时针排点，按照逆时针的绘制方式，绘出来的就是背面
